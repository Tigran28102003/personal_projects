...
\begin{itemize}\setlength\itemsep{4pt}
  ...
  \item \textbf{Ensemble} — взвешенный ансамбль CB$\times$0.6 + LGB$\times$0.4
...
\vspace{-8pt}
\begin{center}
\includegraphics[width=0.88\textwidth,height=0.76\textheight,keepaspectratio,trim=0 30 0 30,clip]{../grp_forecast/figures/metrics_table.png}
\end{center}
\vspace{-4pt}
\centering\scriptsize Зелёным — лучшее значение. OOS-$R^2>0$: модель превосходит наивный бенчмарк.
\end{frame}
...
\end{columns}
\vspace{2pt}
\small \textbf{\#1:} волатильность USD/RUB (\texttt{usd\_rub\_volatility\_lag1}), отрыв вдвое от следующего. Далее: стоимость прод.~корзины, ключевая ставка (ном.~и~реал.), доля платных услуг. MACRO\_BLOCK и PRICE\_BLOCK доминируют над отраслевыми показателями.
\end{frame}
...
\vspace{-4pt}
\small
MACRO\_BLOCK (0.21--0.27) и PRICE\_BLOCK (0.16--0.21) доминируют во всех типах регионов.
PRODUCTION\_BLOCK сильнее для сырьевых (0.17 vs 0.08--0.11). CATEGORICAL\_BLOCK~=~0 везде~--- тип региона не добавляет предсказательной силы сверх количественных признаков.
\end{frame}
...
\end{center}
\vspace{-2pt}
\small A0 — только лаги ВРП; A3 — полный набор признаков. Добавление производственного блока (A0→A1) даёт заметное улучшение. Остальные блоки (A1→A3) вносят меньший вклад для LightGBM. Пунктир — наивный прогноз.
\end{frame}
...
\begin{itemize}\setlength\itemsep{4pt}\small
  \item Лучшая модель: Ensemble(CB$\times$0.6+LGB$\times$0.4),
        RMSE~=~0.0387 (-7.6\% от наивного), OOS-$R^2$~=~0.126
  \item Одиночные: LGBM $\approx$ Ensemble; CatBoost~--- 3-й
  \item Волатильность USD/RUB и ключевая ставка~--- топ предикторы (SHAP)
  \item Устойчивость к выбросам; ошибки выше в кризисные периоды
\end{itemize}
\column{0.42\textwidth}
