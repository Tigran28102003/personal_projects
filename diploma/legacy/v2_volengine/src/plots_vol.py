...
def plot_vol_timing(out: pd.DataFrame, name: str = "m5_vol_timing_equity.png") -> Path:
    """Compounded equity (vol-timing vs buy&hold) on top + the inverse-vol exposure below."""
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(11, 5.2), sharex=True,
                                   gridspec_kw={"height_ratios": [3, 1]})
    ax1.plot(out.index, out["equity_bh"], lw=0.9, color="#999999", label="buy & hold")
    ax1.plot(out.index, out["equity_vt"], lw=1.0, color="#2c7fb8", label="vol-timing (net cost)")
    ax1.set_ylabel("equity (×, compounded)"); ax1.set_yscale("log")
    ax1.set_title("Layer-2 vol-timing overlay vs buy&hold (dev, holdout sealed)")
    ax1.legend(fontsize=8, loc="upper left")
    ax2.fill_between(out.index, 0, out["pos"], color="#2c7fb8", alpha=0.4, step="mid")
    ax2.axhline(config.VOL_TIMING_CAP, color="k", lw=0.6, ls="--", label=f"cap={config.VOL_TIMING_CAP}")
    ax2.set_ylabel("exposure"); ax2.set_ylim(0, max(1.05, config.VOL_TIMING_CAP * 1.05))
