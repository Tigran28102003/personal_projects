...
# free_features.fetch_klines уже умеет пагинированную, кэшированную, graceful
# загрузку Binance klines (теперь с OHLC). Реюзаем её вместо ccxt: новая зависимость
# не нужна, поведение консистентно с остальным data-слоем.
try:  # пакетный импорт (notebook: from src import get_data)
    from .free_features import fetch_klines as _fetch_klines
...
def fetch_binance_ohlcv(
    ...
    """Часовой (или иной) OHLCV BTC с Binance REST с историей с 2017+.

    Тонкая обёртка над :func:`free_features.fetch_klines` (CCXT не нужен): берёт
    open/high/low/close/volume, переименовывает в ``{prefix}Open/High/Low/Close/Volume``
    (downstream-фичи и бэктест ждут ``BTC_Close``/``BTC_Open`` и т.п.). Индекс — UTC
    tz-naive open_time. На сетевой ошибке вернёт пустой df (graceful, как и klines).

    `end`: верхняя отсечка (исключительно); ``None`` -> сегодня. Для воспроизводимости
    передавайте фиксированный ``config.DATA_END``.
    """
    end = end or date.today().strftime("%Y-%m-%d")
    kl = _fetch_klines(symbol, interval, start, end, market=market, use_cache=use_cache)
    if kl.empty:
        cols = [f"{prefix}{c}" for c in ("Open", "High", "Low", "Close", "Volume")]
        return pd.DataFrame(columns=cols)
    out = kl[["open", "high", "low", "close", "volume"]].rename(columns={
        "open": f"{prefix}Open", "high": f"{prefix}High", "low": f"{prefix}Low",
        "close": f"{prefix}Close", "volume": f"{prefix}Volume",
    })
    out.index.name = "Date"
    # отсечка строго < end (klines возвращает по open_time; фиксируем границу)
