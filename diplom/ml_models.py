...
class ForecastBase:
    """
    Base class providing timestamped logging.
    """
...
class LinearForecaster(ForecastBase, BaseEstimator):
    """
    :param test_size: number of last observations for testing
    :param cv_splits: number of TimeSeriesSplit folds
    :param model_type: 'Ridge', 'Lasso', or 'ElasticNet'
    """
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> 'LinearForecaster':
        ...
        self.log(f"Starting {self.model_type} training")
        ...
        # оборачиваем наш пайплайн в TransformedTargetRegressor
        ...
        # теперь все параметры базовой модели идут под префиксом 'regressor__'
        grid = {
...
class StackingForecaster(ForecastBase, BaseEstimator):
    """
    Single-model stacking forecaster на основе sktime и Randomized Search.

    :param str model_type: 'RandomForest', 'CatBoost', или 'XGBoost'
    :param int test_size:    число последних точек для теста
    :param int cv_splits:    число фолдов ExpandingWindowSplitter
    :param int n_iter:       число итераций Randomized Search
